...
% CODE WRITTEN BY AVINASH UPPULURI AND ORIGINALLY PUBLISHED HERE: https://www.mathworks.com/matlabcentral/fileexchange/22187-glcm-texture-features
% ADDITIONAL FEATURES AND MODIFICATIONS WERE ADDED BY RYAN AU.
...
% 5. Zwanenburg, Alex, et al. "Image biomarker standardisation initiative." arXiv preprint arXiv:1612.07003 (2016).
%
% Example:
...
% checked 
out.autoc = zeros(1,size_glcm_3); % Autocorrelation: [2,5]
out.contr = zeros(1,size_glcm_3); % Contrast: matlab/[1,2,5]
out.corrm = zeros(1,size_glcm_3); % Correlation: matlab
out.corrp = zeros(1,size_glcm_3); % Correlation: [1,2,5]
out.cprom = zeros(1,size_glcm_3); % Cluster Prominence: [2,5]
out.cshad = zeros(1,size_glcm_3); % Cluster Shade: [2,5]
out.dent1 = zeros(1,size_glcm_3); % Difference entropy [1]
out.dissi = zeros(1,size_glcm_3); % Dissimilarity: [2,5]
out.dvar1 = zeros(1,size_glcm_3); % Difference variance [4]
out.dvar2 = zeros(1,size_glcm_3); % Difference variance [1]
out.energ = zeros(1,size_glcm_3); % Energy: matlab [1,2,3] called ANGULAR SECOND MOMENT in [1,5], UNIFORMITY in [3]
out.entr1 = zeros(1,size_glcm_3); % Entropy [1,2] called JOINT ENTROPY in [5]
out.homom = zeros(1,size_glcm_3); % Homogeneity: matlab [5] called INVERSE DIFFERENCE in [5]
out.homop = zeros(1,size_glcm_3); % Homogeneity [1,2,5] called INVERSE DIFFERENCE MOMENT in [5]
out.idmnc = zeros(1,size_glcm_3); % Inverse difference moment normalized [3,5]
out.indnc = zeros(1,size_glcm_3); % Inverse difference normalized (INN) [3,5]
out.inf1h = zeros(1,size_glcm_3); % Information measure of correlation1 [1,5]
out.inf2h = zeros(1,size_glcm_3); % Informaiton measure of correlation2 [1,5]
%out.invdc = zeros(1,size_glcm_3);% Inverse difference (INV) is homom [3]
out.maxpr = zeros(1,size_glcm_3); % Maximum probability: [2,5] called JOINT MAXIMUM in [5]
%out.mxcch = zeros(1,size_glcm_3);% maximal correlation coefficient [1]
out.sosvh = zeros(1,size_glcm_3); % Sum of sqaures: Variance [1]
out.savgh = zeros(1,size_glcm_3); % Sum average [1,5]
out.svar1 = zeros(1,size_glcm_3); % Sum variance [1]
out.sent1 = zeros(1,size_glcm_3); % Sum entropy [1]

%added by Ryan Au
out.ctend = zeros(1,size_glcm_3); % Cluster tendency [5]
out.daver = zeros(1,size_glcm_3); % Difference average [5]
out.dent2 = zeros(1,size_glcm_3); % Difference entropy [5]
out.dvar3 = zeros(1,size_glcm_3); % Difference variance [5]
out.entr2 = zeros(1,size_glcm_3); % Entropy with log base 2 [5] called JOINT ENTROPY in [5]
out.joavg = zeros(1,size_glcm_3); % Joint average [5]
out.jovar = zeros(1,size_glcm_3); % Joint variance [5]
out.sent2 = zeros(1,size_glcm_3); % Sum entropy [5]
out.svar2 = zeros(1,size_glcm_3); % Sum variance [5]
%out.invar = zeros(1,size_glcm_3); % Inverse variance [5] excluded because lower bound of j is j>1 instead of j=1
...
%% compute contrast, dissimilarity, energy, entropy, homogeneity, sum of squares: variance, inverse difference
...
for k = 1:size_glcm_3 % number glcms
    ...
    for i = 1:size_glcm_1
        for j = 1:size_glcm_2
            ...
            out.entr1(k) = out.entr1(k) - (glcm(i,j,k)*log(glcm(i,j,k) + eps));
            out.entr2(k) = out.entr2(k) - (glcm(i,j,k)*log2(glcm(i,j,k) + eps));
            ...
            %out.invar(k) = out.invar(k) + (2*(glcm(i,j,k)/(i-j)^2));
            ...
            out.joavg(k) = u_x(k);
            out.jovar(k) = out.jovar(k) + glcm(i,j,k)*((i - u_x(k))^2);
        end
    end
...
%     consider u_x and u_y and s_x and s_y as means and standard deviations of p_x and p_y
%     u_x2(k) = mean(p_x(:,k));
%     u_y2(k) = mean(p_y(:,k));
%     s_x2(k) = std(p_x(:,k));
%     s_y2(k) = std(p_y(:,k));
...
%% compute sum average and sum entropy:
for k = 1:(size_glcm_3)
    for i = 1:(2*(size_glcm_1)-1)
        out.savgh(k) = out.savgh(k) + (i+1)*p_xplusy(i,k);
        % the summation for savgh is for i from 2 to 2*Ng hence (i+1)
        out.sent1(k) = out.sent1(k) - (p_xplusy(i,k)*log(p_xplusy(i,k) + eps));
        out.sent2(k) = out.sent2(k) - (p_xplusy(i,k)*log2(p_xplusy(i,k) + eps));
    end
end
...
for k = 1:(size_glcm_3)
    for i = 1:(2*(size_glcm_1)-1)
        out.svar1(k) = out.svar1(k) + (((i+1) - out.sent1(k))^2)*p_xplusy(i,k);
        out.svar2(k) = out.svar2(k) + (((i+1) - out.savgh(k))^2)*p_xplusy(i,k);
        % the summation for savgh is for i from 2 to 2*Ng hence (i+1)
    end
end

%% compute difference variance and difference entropy
for k = 1:size_glcm_3
out.dvar2(k) = var(p_xminusy(:,k));
% but using the formula in 
% http://murphylab.web.cmu.edu/publications/boland/boland_node26.html
% we have for dvarh
    for i = 0:(size_glcm_1-1)
        out.dent1(k) = out.dent1(k) - (p_xminusy(i+1,k)*log(p_xminusy(i+1,k) + eps));
        out.dent2(k) = out.dent2(k) - (p_xminusy(i+1,k)*log2(p_xminusy(i+1,k) + eps));
        out.daver(k) = out.daver(k) + (i)*p_xminusy(i+1,k);
        out.dvar1(k) = out.dvar1(k) + (i^2)*p_xminusy(i+1,k);
        out.dvar3(k) = out.dvar3(k) + ((i - out.daver(k))^2)*p_xminusy(i+1,k);
    end
end
...
%% compute cluster prominence, cluster shade, autocorrelation, and correlations
...
for k = 1:size_glcm_3
    for i = 1:size_glcm_1
        for j = 1:size_glcm_2
            ...
            corm(k) = corm(k) + (((i) - u_x(k)*(i))*((j) - u_y(k)*(j))*glcm(i,j,k));
            out.cprom(k) = out.cprom(k) + (((i + j - u_x(k) - u_y(k))^4)*glcm(i,j,k));
            out.cshad(k) = out.cshad(k) + (((i + j - u_x(k) - u_y(k))^3)*glcm(i,j,k));
            out.ctend(k) = out.ctend(k) + (((i + j - u_x(k) - u_y(k))^2)*glcm(i,j,k));
        end
    end
    % using http://www.fp.ucalgary.ca/mhallbey/glcm_variance.htm for s_x
    % s_y: This solves the difference in value of correlation and might be right value of standard deviations required 
    % According to this website there is a typo in [2] which provides values of variance instead of the 
    % standard deviation hence a square root is required as done below:
    s_x(k) = s_x(k) ^ 0.5;
    s_y(k) = s_y(k) ^ 0.5;
    out.autoc(k) = corp(k);
